...
import { getErrorMessage, isUpcomingEventDate, parseEventDate } from '../../lib/content';
...
export function Events() {
  ...
  async function fetchEvents() {
    ...
    if (error) {
      ...
      setMessage({ type: 'error', text: getErrorMessage(error, 'Tapahtumien lataus epäonnistui.') });
    } else {
      setEvents(data || []);
    }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (startTime && endTime && endTime <= startTime) {
      setMessage({ type: 'error', text: 'Päättymisajan on oltava alkamisajan jälkeen.' });
      return;
    }
    ...
    if (isMockSupabase) {
      if (editingId) {
        setEvents((prev) => prev.map((event) => (
          event.id === editingId ? { ...event, ...payload, updated_at: new Date().toISOString() } : event
        )));
        ...
        setEvents([...events, newEvent].sort((a, b) => parseEventDate(a.event_date).getTime() - parseEventDate(b.event_date).getTime()));
    ...
    if (editingId) {
      ...
      if (error) {
        setMessage({ type: 'error', text: getErrorMessage(error, 'Tapahtuman päivitys epäonnistui.') });
        return;
      }
      ...
      if (error) {
        setMessage({ type: 'error', text: getErrorMessage(error, 'Tapahtuman luonti epäonnistui.') });
        return;
      }
  ...
  async function handleDelete(id: string) {
    ...
    if (error) {
      setMessage({ type: 'error', text: getErrorMessage(error, 'Tapahtuman poisto epäonnistui.') });
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        {loading ? (
          ...
          events.map((event) => {
            const now = new Date();
            const isPast = !isUpcomingEventDate(event.event_date, now);
              ...
              <Card key={event.id} className={!isActive ? 'opacity-60' : ''}>
                <CardContent className="flex items-start justify-between p-6">
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500">
                      <div className="flex items-center">
                        ...
                        {format(parseEventDate(event.event_date), 'd.M.yyyy', { locale: fi })}
                      </div>
                  ...
                  <div className="ml-4 flex items-center space-x-2">
                    ...
                    <Button variant="outline" size="icon" className="text-red-500 hover:text-red-600" onClick={() => void handleDelete(event.id)}>
